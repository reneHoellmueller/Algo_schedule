from .models import Employee, OpeningTime, ShiftPreference, NonWorkingDay
...
def generate_binary_schedule(year, month, club_id, employee_id_to_idx):
    ...
    non_working_days = NonWorkingDay.objects.filter(employee__in=employees)
    ...
    non_working_dict = {}
    for non_working in non_working_days:
        if non_working.employee_id not in non_working_dict:
            non_working_dict[non_working.employee_id] = set()
        non_working_dict[non_working.employee_id].add(non_working.date)
        ...
        for hour in range(24):
            if opening_time.opening_time.hour <= hour < opening_time.closing_time.hour:
                for employee in employees:
                    ...
                    if current_date.date() in non_working_dict.get(employee.id, set()):
                        binary_schedule[hour][day - 1][employee_idx] = 0
                        continue
